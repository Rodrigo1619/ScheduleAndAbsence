...
public interface ScheduleRepository extends JpaRepository<Schedule, UUID> {
    ...
    @Query("SELECT s FROM Schedule s WHERE s.user_x_subject.user.id = :userId AND s.classroomConfiguration.classroom.year = :year AND s.classroomConfiguration.classroom.shift.id = :shiftId")
    List<Schedule> findSchedulesByUserAndShiftAndYear(@Param("userId") UUID userId, @Param("shiftId") UUID shiftId,
            @Param("year") String year);
